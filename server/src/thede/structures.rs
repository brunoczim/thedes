...
impl House {
    ...
    pub async fn spawn(self, map: &mut Map) -> Result<()> {
        for coord in self.rect.rows() {
            map.entry_mut(coord).await?.block = Block::Empty;
        }
...
impl Village {
    pub async fn spawn(&self, map: &mut Map) -> Result<()> {
        self.spawn_houses(map).await?;
        self.spawn_path(map).await?;
        Ok(())
    }

    async fn spawn_houses(&self, map: &mut Map) -> Result<()> {
    ...
    async fn spawn_path(&self, map: &mut Map) -> Result<()> {
        for (vertex_a, vertex_b) in self.paths.connections() {
            ...
            for axis in Axis::iter() {
                ...
                for coord in range {
                    ...
                    map.entry_mut(path_point).await?.ground = Ground::Path;
                }
            }
        }
        Ok(())
